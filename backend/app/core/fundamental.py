...
# Bug #6: cache the expensive yfinance fundamentals payload.
def _load_fundamentals_cached(ticker: str) -> dict:
    ...
    payload = {
        "info": dict(stock.info) if getattr(stock, "info", None) else {},
        "financials_records": (
            stock.financials.reset_index().to_dict(orient="records")
            if stock.financials is not None and not stock.financials.empty else []
        ),
        "balance_records": (
            stock.balance_sheet.reset_index().to_dict(orient="records")
            if stock.balance_sheet is not None and not stock.balance_sheet.empty else []
        ),
        "cashflow_records": (
            stock.cashflow.reset_index().to_dict(orient="records")
            if stock.cashflow is not None and not stock.cashflow.empty else []
        ),
    }
    try:
        cache.set(key, payload, ttl=settings.CACHE_TTL_FUNDAMENTALS)
    except Exception as e:
        print(f"⚠️  Fundamentals cache write failed: {e}")
    return payload


# Bug #11: yfinance reports debtToEquity as a percent (e.g. 170 means 170%).
# Normalise to a decimal ratio (1.7) so downstream thresholds are sane and
# comparable across yfinance versions / sources.
def _normalize_debt_to_equity(value) -> Optional[float]:
    ...
    try:
        ...
        return None
    # Heuristic: anything > 5 is almost certainly the percent form.
...
class FundamentalAnalyst:
    ...
    def _load_data(self):
        """Load basic stock data and financial statements (Bug #6: served from cache)."""
        try:
            payload = _load_fundamentals_cached(self.ticker)
            ...
            def _records_to_df(records):
                ...
                # First column on yfinance statements is the row label index
            ...
            self.financials = _records_to_df(payload.get("financials_records"))
            self.balance_sheet = _records_to_df(payload.get("balance_records"))
            self.cash_flow = _records_to_df(payload.get("cashflow_records"))
        except Exception as e:
    ...
    def get_key_financial_metrics(self) -> Dict[str, Any]:
        ...
        # Bug #11: surface as decimal ratio so users see "1.70" not "170"
        normalized_de = _normalize_debt_to_equity(self.info.get("debtToEquity"))
        metrics["Debt to Equity"] = round(normalized_de, 2) if normalized_de is not None else "N/A"
    ...
    def generate_investment_score(self) -> Dict[str, Any]:
        ...
        # Debt to Equity scoring (Bug #11: normalize to decimal ratio first; gate
        # on D/E < 1.0, the textbook "manageable" threshold).
        debt_to_equity = _normalize_debt_to_equity(self.info.get("debtToEquity"))
        if debt_to_equity is not None and debt_to_equity < 1.0:
            score += 1
            reasons.append(f"Manageable debt levels (D/E ≈ {debt_to_equity:.2f})")
        elif debt_to_equity is not None and debt_to_equity >= 2.0:
            reasons.append(f"Elevated leverage (D/E ≈ {debt_to_equity:.2f})")
