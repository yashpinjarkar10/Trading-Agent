...
# Bug #6: cache OHLCV downloads. Bucket by ~5-minute window so two near-simultaneous
# requests share data, but data is never older than CACHE_TTL_OHLCV.
def _load_history_cached(ticker: str, period: str) -> pd.DataFrame:
    bucket = int(datetime.utcnow().timestamp() // 300)  # 5-min bucket
    key = f"ohlcv:{ticker}:{period}:{bucket}"
    cached_df = cache.get(key)
    if cached_df is not None:
        try:
            # Stored as records list — rebuild DataFrame with DatetimeIndex
            df = pd.DataFrame(cached_df["data"])
            if not df.empty and "Date" in df.columns:
                df["Date"] = pd.to_datetime(df["Date"])
                df = df.set_index("Date")
            return df
        except Exception:
            pass  # fall through to refetch on cache corruption
    ...
    if df is not None and not df.empty:
        try:
            ...
            cache.set(key, {"data": payload}, ttl=settings.CACHE_TTL_OHLCV)
        except Exception as e:
            print(f"⚠️  OHLCV cache write failed: {e}")
    return df


# Bug #12: minimum bars required to trust each window length.
def _safe_latest(series: pd.Series, min_periods: int = 1):
    """Return the last non-NaN value if there are enough bars, else None."""
...
class TechnicalAnalyst:
    ...
    def _load_data(self):
        """Load historical stock data (Bug #6: served from cache when possible)."""
        try:
            # Get historical data — cached at the module level
            self.data = _load_history_cached(self.ticker, self.period)
    ...
    def detect_signals(self) -> List[TechnicalSignal]:
        ...
        # Moving Average Signals (Bug #12: skip if not enough bars for either window)
        sma_50 = _safe_latest(self.indicators['sma_50'], min_periods=50)
        sma_200 = _safe_latest(self.indicators['sma_200'], min_periods=200)
    ...
    def get_current_trend(self) -> Dict[str, Any]:
        ...
        latest_close = self.data['Close'].iloc[-1]
        # Bug #12: window-length minimums so signals reflect real data, not NaN.
        sma_20 = _safe_latest(self.indicators['sma_20'], min_periods=20)
        sma_50 = _safe_latest(self.indicators['sma_50'], min_periods=50)
        sma_200 = _safe_latest(self.indicators['sma_200'], min_periods=200)

        def _classify(close, sma):
            if sma is None or pd.isna(sma):
                return "Insufficient Data"
            return "Bullish" if close > sma else "Bearish"

        short_term = _classify(latest_close, sma_20)
        medium_term = _classify(latest_close, sma_50)
        long_term = _classify(latest_close, sma_200)

        # Only count classifications backed by real data toward the overall trend.
        valid = [t for t in (short_term, medium_term, long_term) if t in {"Bullish", "Bearish"}]
        ...
        sma_alignment = (
            sma_20 is not None and sma_50 is not None and sma_200 is not None
            and sma_20 > sma_50 > sma_200
        )
