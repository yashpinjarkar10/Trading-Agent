...
class NewsAnalyst:
    ...
    def analyze_sentiment(
        ...
        """
        Bug #3: industry-style sentiment WITHOUT an LLM.

        Backed by `app.core.sentiment.analyze_sentiment` which combines:
          - VADER lexicon-rule analyzer (handles negation, intensifiers,
            "not beat" vs "beat", punctuation, capitalization).
          - Loughran-McDonald style finance lexicon overlay
            (downgrade, miss, profit warning, raised guidance, buyback…).
          - Multi-word phrase patterns ("guidance cut", "fda approval").
          - Source-quality and time-decay weighting.
        """
        from app.core.sentiment import analyze_sentiment as _impl

        result = _impl(text, publisher=publisher, source=source, published=published)
        return {
    ...
    def get_comprehensive_news_analysis(self, days_back: int = 7, max_articles: int = 50) -> Dict[str, Any]:
        ...
        for article in unique_articles:
            ...
            # Bug #3: pass publisher / source / published so the new sentiment
            # engine can apply source-quality and time-decay weighting.
            ...
            # Weight by relevance × the engine's already-applied source/time weight
